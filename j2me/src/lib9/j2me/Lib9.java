...
/**
 * Lib9引擎的核心类，可以看成是一个状态机，程序的各个部分(每个状态即实现了L9IState接口类的实例)都被装入这个状态机得以运行，
 * 同时该类还实现了对按键系统支持，通过对按键体系的分离，使得可以在每个状态中去判断按键(与用户交互)，大大简化的程序的开发
 * 
 * @author not attributable
 * @version 1.0
 */

public class Lib9 extends Canvas implements Runnable {
	/**
	 * 屏幕的宽
	 */
	public int SCR_W;
	...
	/**
	 * 切换到上一次的显示容器，调用setDisplay设置显示容器的时候将会记录上一次显示容器，
	 * 调用backLastDisplay返回上一次显示容器后会将lastAppDisplay=null
	 */
	...
	/**
	 * 在帧驱动的应用或者游戏(帧驱动的方式往往是游戏)中每一帧都要清除按键值，isKeyPressed,isKeyReleased将会受到影响,
	 * 而isKeyUp,isKeyDown则需要手动 调用
	 */
	...
	/**
	 * 判断某个键是否已经被释放了，注意参数是逻辑键值，
	 * 这个函数与isKeyReleased的区别在于在应用程序的每一帧isKeyPressed对应的键值都会被清除，
	 ...
	 * 这个函数与isKeyPressed的区别在于在应用程序的每一帧isKeyPressed对应的键值都会被清除，
	 ...
	 * @return boolean
	 */
	public boolean isKeyPressed(int logicKey) {
